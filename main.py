...
mask = (test_gr['year'].isnull() == False) & (test_gr['universities'].isnull() == False) & (test_gr['universities'].str.contains(str_of_best, na=False)) & (test_gr['year'] < 2008) & (2002 < test_gr['year'])
...
mask2 = (test_gr['year'].isnull() == False) & (test_gr['universities'].isnull() == False) & (test_gr['universities'].str.contains(str_of_best, na=False) == False) & (test_gr['year'] < 2008) & (2002 < test_gr['year'])
...
mask_contr = (contr_gr['year'].isnull() != True) & (contr_gr['universities'].isnull() != True) & (contr_gr['universities'].str.contains(str_of_best, na=False)) & (contr_gr['year'] < 2008) & (2002 < contr_gr['year']) & ((contr_gr['id'].isin(test_gr['id'])) == False)
...
mask_contr2 = (contr_gr['year'].isnull() != True) & (contr_gr['universities'].isnull() != True) & (contr_gr['universities'].str.contains(str_of_best, na=False) != True) & (contr_gr['year'] < 2008) & (2002 < contr_gr['year']) & ((contr_gr['id'].isin(test_gr['id'])) == False)
...
print(final_test[['data_from', 'universities']].corr(method="spearman"))
...
plt.legend(['Обычный', 'Топ'], loc='upper right')
plt.show()
